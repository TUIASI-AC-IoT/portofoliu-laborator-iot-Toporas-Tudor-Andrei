...
static void udp_task(void *pvParameters)
{
    ...
    while(1)
    {
        ...
        // send_buffer[6] = '0';//turn off
        // send_buffer[6] = '1';//turn on
        //int ret = sendto(sock, send_buffer, sizeof(send_buffer), 0, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
        ...
        while (1) {
            ...
            // Data received
                ...
                {
                case '0':
                    ESP_LOGI(TAG, "%s => Turning LED OFF", rx_buffer);
                    gpio_set_level(GPIO_OUTPUT_IO, 1);
                    break;
                case '1':
                    ESP_LOGI(TAG, "%s => Turning LED ON", rx_buffer);
                    gpio_set_level(GPIO_OUTPUT_IO, 0);
                    break;
                default:
                    ESP_LOGE(TAG, "Invalid Parameter: %s", rx_buffer);
                    break;
                }
            }
