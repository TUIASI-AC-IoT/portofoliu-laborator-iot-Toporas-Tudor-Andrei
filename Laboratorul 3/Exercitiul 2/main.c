...
#define CONFIG_EXAMPLE_FIRMWARE_UPGRADE_URL "https://192.168.253.229:5000/firmware.bin"
#define CONFIG_EXAMPLE_VERSION_URL "https://192.168.253.229:5000/version"
...
esp_err_t _http_event_handler(esp_http_client_event_t *evt)
{
    ...
    case HTTP_EVENT_ON_DATA:
        ...
        if ( atoi(BUILD_NUMBER) < atoi(evt->user_data) ){
            xEventGroupSetBits(s_event_start_ota, BIT_OLD_VERSION);
        }
        break;
...
{
    while (1) {
        xEventGroupWaitBits(s_event_start_ota, BIT_OLD_VERSION, pdTRUE, pdTRUE, portMAX_DELAY);
...
static void check_version_task(void * pvParameter){
    
    while (1) {
        xEventGroupWaitBits(s_event_start_ota, BIT_BTN_PRESSED, pdTRUE, pdTRUE, portMAX_DELAY);

        ESP_LOGI(TAG, "Starting check_version task");
        esp_http_client_config_t config = {
...
void app_main(void)
{
    ...
    if (connected) {
        ...
        xTaskCreate(check_version_task, "check_version_task", 8192, NULL, 5, NULL);
    }
}